...
import { ReopenTicketCommandHandler } from '../reopen-ticket.command-handler';
...
describe('ReopenTicketCommandHandler', () => {
  let handler: ReopenTicketCommandHandler;
  ...
  beforeEach(async () => {
    ...
    handler = testingModule.get(ReopenTicketCommandHandler);
  ...
  it('should reopen and save when ticket is done', async () => {
    const ticket = makeDoneTicket();
    writeRepo.findById.mockResolvedValueOnce(Result.success(ticket));

    const result = await handler.execute(new ReopenTicketCommand(TICKET_ID));

    expect(result.isSuccess()).toBe(true);
    expect(writeRepo.save.mock.calls).toHaveLength(1);
    expect(ticket.getProps().status.value).toBe('reopened');
  });
  ...
  it('should return domain failure when ticket is not done', async () => {
    const ticket = makeOpenTicket();
    writeRepo.findById.mockResolvedValueOnce(Result.success(ticket));
