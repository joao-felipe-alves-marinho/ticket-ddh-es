...
import { ResolveTicketCommandHandler } from '../resolve-ticket.command-handler';
...
describe('ResolveTicketCommandHandler', () => {
  let handler: ResolveTicketCommandHandler;
  ...
  beforeEach(async () => {
    ...
    handler = testingModule.get(ResolveTicketCommandHandler);
  ...
  it('should resolve and save when ticket is in progress', async () => {
    ...
    const result = await handler.execute(new ResolveTicketCommand(TICKET_ID));

    expect(result.isSuccess()).toBe(true);
    expect(writeRepo.save.mock.calls).toHaveLength(1);
    expect(ticket.getProps().status.value).toBe('done');
  });
