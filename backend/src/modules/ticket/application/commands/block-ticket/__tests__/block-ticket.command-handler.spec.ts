...
import { BlockTicketCommandHandler } from '../block-ticket.command-handler';
...
const BLOCK_REASON = 'Waiting for dependency from auth team';
...
describe('BlockTicketCommandHandler', () => {
  let handler: BlockTicketCommandHandler;
  ...
  beforeEach(async () => {
    ...
    handler = testingModule.get(BlockTicketCommandHandler);
  ...
  it('should block and save when ticket is in progress', async () => {
    ...
    const result = await handler.execute(
      new BlockTicketCommand(TICKET_ID, BLOCK_REASON),
    );

    expect(result.isSuccess()).toBe(true);
    expect(writeRepo.save.mock.calls).toHaveLength(1);
    expect(ticket.getProps().status.value).toBe('blocked');
  });
